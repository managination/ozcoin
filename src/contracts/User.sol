...
contract User is BaseContract{
    ...
    function setRole(address _account, Role _role) external {
        users[_account].role = _role;
    ...
    function updateUserDetails(address _account, bytes32 _name,string newDetails,address _affiliateAccount,address _affiliateCompany) userActiveStatus(_account, true) {
       if(_account == msg.sender || users[msg.sender].role == Role.Administrator) {
           users[_account].details = newDetails;
           users[_account].name = _name;
           users[_account].affiliateAccount = _affiliateAccount;
           users[_account].affiliateCompany = _affiliateCompany;
           UserDetailsChanged(_account);
           AffiiateSet(_account, _affiliateAccount);
           AffiiateCompanySet(_account, _affiliateCompany);
        }
